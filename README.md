...
Full-features shell script in order to manage Renkforce's BS-21 bluetooth switch and scheduler 
...
* 20 power-on schedulers and 20 power-off schedulers which can be individually assigned to weekdays
...
$ bs21 --help
 ...
 --scheduler <n:1-20> <on|off> <mtwtfss> <hh:mm>        sets specific scheduler (1-20) with action (turn on / turn off), daymask, e.g. MTWTFss for Monday to Friday, starttime
 --scheduler-clear <n:1-20> <on|off>    resets specific scheduler
 --schedulers                           prints all scheduler information
...
You can query the remaining time of the countdown by using `--schedulers`:
```
$ bs21.py EndOfWorkSiren --schedulers
...
```
In this example random mode will be set for Saturday at 9:30 for 2:00 hours. The day mask is compiled by the first letter for every day. Capital letters indicate that scheduler will be set for this day. 

Random mode can be stopped by calling `--random-clear`

## Schedulers
The bluetooth switch has 40 scheduler slots: 20 for turning power on and another 20 for turning power off. On and off schedulers are totally independent. 

Program a scheduler for starting your coffeemachine from Monday to Friday at 7:10 a.m.
```
$ bs21.py Coffee --scheduler 1 on MTWTFss 7:10
```

Don't forget to turn coffeemachine off afterwards:
```
$ bs21.py Coffee --scheduler 1 off MTWTFss 7:20
```

At weekend I would like to have coffee at 8:00 a.m.
```
$ bs21.py Coffee --scheduler 2 on mtwtfSS 8:00
$ bs21.py Coffee --scheduler 2 off mtwtfSS 8:10
```

Please notice the day mask. The day mask is compiled by the first letter for every day. Capital letters indicate that scheduler will be set for this day. 

Let's take a look at the schedulers that we have set so far:
```
$ bs21.py Coffee --schedulers
 Scheduler 01:         Switch on at 07:10 on Mon, Tue, Wed, Thu, Fri
 Scheduler 02:         Switch on at 08:00 on Sat, Sun
 Scheduler 01:         Switch off at 07:16 on Mon, Tue, Wed, Thu, Fri
 Scheduler 02:         Switch off at 08:10 on Sat, Sun
```

You can clear a specific scheduler as follows:
```
$ bs21.py Coffee --scheduler-clear 2
```
 
Last but not least you can clear all schedules, i.e. schedulers, random mode and countdown by calling `--clear-all`.
```
...
If you want to get the full status and scheduler information in JSON format call this:
```
$ bs21.py Light --json
{
  "status": {
